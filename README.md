...
Mill rules and search are implemented in Rust (`crates/tgf-mill`,
`crates/tgf-search`).  The legacy **perfect-play database** and **opening book**
remain in C++ and are linked through the transitional `crates/tgf-legacy-cxx`
bridge (they are not ported to Rust by design).
