...
Sanmill's supported frontend is the Flutter app.  The legacy Qt UI was
removed together with the legacy C++ engine layer; debugging and headless
engine work now use the Rust CLI in `crates/tgf-cli`.
...
pruning, MTD(f), MCTS, and iterative deepening.  The legacy C++ engine and
Qt UI have been removed.  The perfect database is kept as a vendored C++
library behind the Rust `perfect-db` wrapper.
