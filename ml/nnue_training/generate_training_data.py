...
def generate_training_data_with_perfect_db(perfect_db_path: str, 
    ...
    try:
        ...
        with tqdm(total=num_positions, desc="Generating positions", 
                 ...
                 mininterval=10.0,  # 最少10秒才刷新一次
                 maxinterval=30.0,  # 最多30秒强制刷新一次
                 miniters=100000) as pbar:  # 最少100000次更新才刷新
            ...
            def progress_callback(current_count):
                # Only update progress bar every 1000 positions to avoid spam
                if current_count - last_update[0] >= 1000 or current_count >= num_positions:
                    pbar.n = current_count
                    pbar.refresh()  # Force refresh only when we actually want to update
                    last_update[0] = current_count
            
            positions = generate_positions_from_sectors(pdb, game, sectors_by_phase, num_positions, progress_callback)
            # Final update to show completion - let tqdm handle the display
            pbar.update(len(positions) - pbar.n)
        ...
        with tqdm(total=len(positions), desc="Evaluating positions", 
                 ...
                 mininterval=10.0,  # 最少10秒才刷新一次
                 maxinterval=30.0,  # 最多30秒强制刷新一次
                 miniters=1000) as pbar:  # 评估阶段每1000次更新才刷新
        ...
        with tqdm(total=valid_positions + 5, desc="Writing data file", 
            ...
            with open(output_file, 'w') as f:
                ...
                # Write training data (update progress every 100000 lines)
                lines_written = 0
                for i, line in enumerate(training_data):
                    f.write(line)
                    lines_written += 1
                    
                    # Update progress every 100000 lines or at the end
                    if lines_written == 100000 or i == len(training_data) - 1:
                        pbar.update(lines_written)
                        lines_written = 0
