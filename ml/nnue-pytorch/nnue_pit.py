...
class NNUEGameGUI:
    ...
    def start_gui(self):
        ...
        self.root.geometry("700x850")  # Increased height to accommodate evaluation display
    ...
    def get_human_perspective_evaluation(self) -> float:
        """Calculate position evaluation from Human player's perspective"""
        if self.game_over:
            return 0.0
        ...
        # NNUE model returns evaluation from current player's perspective
        # side_to_move: 0=white to move, 1=black to move
        # raw_evaluation > 0 means favorable for current player to move
        
        # First convert to white's perspective evaluation
        if self.game_state.side_to_move == 0:
            # Currently white to move, raw_evaluation is already from white's perspective
            white_perspective_eval = raw_evaluation
        else:
            # Currently black to move, raw_evaluation is from black's perspective, negate to get white's perspective
            white_perspective_eval = -raw_evaluation
        
        # Then convert to Human's perspective
        if self.human_player_value == 1:
            # Human is white, use white's perspective evaluation
            return white_perspective_eval
        else:
            # Human is black, negate white's perspective evaluation
            return -white_perspective_eval
    ...
    def update_evaluation_display(self):
        ...
        # Calculate Human's perspective evaluation
        ...
        if abs(self.current_evaluation) > 10:
            ...
            # Clear advantage
            eval_text = f"NNUE Evaluation: {self.current_evaluation:+.1f} (Clear advantage)"
        elif abs(self.current_evaluation) > 1:
            # Slight advantage
            eval_text = f"NNUE Evaluation: {self.current_evaluation:+.1f} (Slight advantage)"
        else:
            # Equal position
    ...
    def draw_evaluation_bar(self):
        """Draw evaluation value progress bar"""
        self.eval_canvas.delete("all")
        ...
        # Map evaluation value to [-1, 1] range using tanh function for smooth mapping
        import math
        normalized_eval = math.tanh(self.current_evaluation / 3.0)  # 3.0 is scaling factor
        
        # Calculate progress bar position
        center_x = bar_width // 2
        bar_position = center_x + (normalized_eval * center_x * 0.9)  # 0.9 for margin
