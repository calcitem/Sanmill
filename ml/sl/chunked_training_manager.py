...
class ChunkedCheckpointManager:
    """Enhanced checkpoint manager for chunked training with anomaly tracking and per-chunk saving."""
    
    def __init__(self, checkpoint_dir: str, keep_n_checkpoints: int = 5, save_per_chunk: bool = True):
        """
        ...
        Args:
            ...
            save_per_chunk: Whether to save checkpoint after each chunk completion
        """
        ...
        self.chunk_checkpoint_dir = self.checkpoint_dir / "chunks"
        self.epoch_checkpoint_dir.mkdir(exist_ok=True)
        self.chunk_checkpoint_dir.mkdir(exist_ok=True)
        
        # Track checkpoint health
        self.checkpoint_health = {}  # epoch -> health_score
        self.chunk_checkpoint_health = {}  # (epoch, chunk) -> health_score
        self.last_healthy_checkpoint = None
        
        logger.info(f"ChunkedCheckpointManager initialized: {checkpoint_dir}")
        logger.info(f"Per-chunk saving: {'Enabled' if save_per_chunk else 'Disabled'}")
    
    def save_chunk_checkpoint(self, neural_network, epoch: int, chunk_idx: int, 
                             optimizer_state: Dict, loss_history: List[float], 
                             config: Dict[str, Any], warning_stats: Dict[str, int], 
                             training_metrics: Dict[str, Any], chunk_stats: Dict[str, Any]) -> str:
        """
        Save a checkpoint after completing a chunk.
        ...
        Args:
            ...
            chunk_stats: Statistics specific to this chunk
            
        Returns:
            Path to saved checkpoint
        """
        if not self.save_per_chunk:
            return ""
            
        checkpoint_path = self.chunk_checkpoint_dir / f"chunk_checkpoint_e{epoch}_c{chunk_idx}.tar"
        
        # Calculate chunk-specific health score
        health_score = self._calculate_chunk_health_score(chunk_stats, warning_stats, training_metrics)
        
        # Save the neural network
        neural_network.save(str(checkpoint_path))
        
        # Save chunk-specific training state
        state_path = self.chunk_checkpoint_dir / f"chunk_training_state_e{epoch}_c{chunk_idx}.json"
        training_state = {
            'epoch': epoch,
            'chunk_idx': chunk_idx,
        ...
        # Update chunk health tracking
        self.chunk_checkpoint_health[(epoch, chunk_idx)] = health_score
        
        # Display chunk checkpoint info
        health_status = "✅ Healthy" if health_score > 0.7 else "⚠️ Warning" if health_score > 0.4 else "🚨 Poor"
        print(f"💾 Chunk checkpoint saved: Epoch {epoch}, Chunk {chunk_idx}")
        print(f"   Health Score: {health_score:.2f} ({health_status})")
        print(f"   Chunk Loss: {chunk_stats.get('loss', 0):.6f}")
    ...
    def save_checkpoint(self, neural_network, epoch: int, optimizer_state: Dict,
        ...
        Returns:
            Path to saved checkpoint
        """
        checkpoint_path = self.epoch_checkpoint_dir / f"chunked_checkpoint_epoch_{epoch}.tar"
    ...
    def get_latest_checkpoint(self) -> Optional[str]:
        """Get the path to the latest checkpoint (epoch or chunk)."""
        # Check for epoch checkpoints
        epoch_pattern = self.epoch_checkpoint_dir / "chunked_checkpoint_epoch_*.tar"
        epoch_checkpoints = glob.glob(str(epoch_pattern))
        
        # Check for chunk checkpoints
        chunk_pattern = self.chunk_checkpoint_dir / "chunk_checkpoint_*.tar"
        chunk_checkpoints = glob.glob(str(chunk_pattern))
        ...
        if epoch_checkpoints:
            def extract_epoch(path):
                try:
                    ...
                    return 0
            latest_epoch_checkpoint = max(epoch_checkpoints, key=extract_epoch)
        
        if chunk_checkpoints:
            def extract_epoch_chunk(path):
                try:
                    filename = Path(path).stem
                    # Format: chunk_checkpoint_e{epoch}_c{chunk}
                    parts = filename.split('_')
                    epoch = int(parts[-2][1:])  # Remove 'e' prefix
                    chunk = int(parts[-1][1:])  # Remove 'c' prefix
                    return (epoch, chunk)
                except (ValueError, IndexError):
                    return (0, 0)
            latest_chunk_checkpoint = max(chunk_checkpoints, key=extract_epoch_chunk)
        
        # Determine which is more recent
        if latest_epoch_checkpoint and latest_chunk_checkpoint:
            epoch_num = self._extract_epoch_from_path(latest_epoch_checkpoint)
            chunk_epoch, chunk_idx = extract_epoch_chunk(latest_chunk_checkpoint)
            
            # Prefer chunk checkpoint if it's from the same or later epoch
            if chunk_epoch >= epoch_num:
                return latest_chunk_checkpoint
            else:
                return latest_epoch_checkpoint
        elif latest_chunk_checkpoint:
            return latest_chunk_checkpoint
    ...
    def load_checkpoint(self, neural_network, checkpoint_path: str) -> Dict[str, Any]:
        ...
        # Determine checkpoint type and load appropriate state
        checkpoint_path_obj = Path(checkpoint_path)
        
        if "chunk_checkpoint" in checkpoint_path_obj.name:
            # This is a chunk checkpoint
            return self._load_chunk_checkpoint_state(checkpoint_path)
        else:
            # This is an epoch checkpoint
            return self._load_epoch_checkpoint_state(checkpoint_path)
    
    def _load_epoch_checkpoint_state(self, checkpoint_path: str) -> Dict[str, Any]:
        """Load state for an epoch checkpoint."""
        epoch = self._extract_epoch_from_path(checkpoint_path)
        state_path = self.epoch_checkpoint_dir / f"chunked_training_state_epoch_{epoch}.json"
    ...
    def _load_chunk_checkpoint_state(self, checkpoint_path: str) -> Dict[str, Any]:
        """Load state for a chunk checkpoint."""
        # Extract epoch and chunk from path
        checkpoint_path_obj = Path(checkpoint_path)
        filename = checkpoint_path_obj.stem
        ...
        if state_path.exists():
            ...
            # Verify chunk checkpoint health
            health_score = training_state.get('health_score', 0.5)
            is_healthy = training_state.get('is_healthy', False)
            
            print(f"📂 Loaded chunk checkpoint from epoch {epoch}, chunk {chunk_idx}")
            print(f"   Health Score: {health_score:.2f}")
            print(f"   Status: {'✅ Healthy' if is_healthy else '⚠️ Has Issues'}")
            
            if not is_healthy:
                print("⚠️ WARNING: This chunk checkpoint was saved during problematic training")
                print("   Consider using a healthier checkpoint if available")
            
            return training_state
        else:
            # Fallback for chunk checkpoints without detailed state
            return {
                'epoch': epoch,
                'chunk_idx': chunk_idx,
                'optimizer_state_dict': {},
    ...
    def _calculate_chunk_health_score(self, chunk_stats: Dict[str, Any], 
                                    warning_stats: Dict[str, int], 
                                    training_metrics: Dict[str, Any]) -> float:
        """
        Calculate health score for a chunk checkpoint (0.0 = poor, 1.0 = excellent).
        
        Args:
            chunk_stats: Chunk-specific statistics
            warning_stats: Warning statistics
        ...
        # Loss-based scoring
        chunk_loss = chunk_stats.get('loss', float('inf'))
        if chunk_loss > 10.0:
            score -= 0.5  # Very high loss
        elif chunk_loss > 1.0:
            score -= 0.3  # High loss
        elif chunk_loss > 0.1:
            score -= 0.1  # Moderate loss
        ...
        memory_usage = training_metrics.get('memory_usage_percent', 0)
        if memory_usage > 90:
            score -= 0.2
        elif memory_usage > 80:
            score -= 0.1
        
        # Training speed bonus/penalty
        samples_per_second = training_metrics.get('samples_per_second', 0)
        if samples_per_second > 500:
            score += 0.1  # Good performance
        elif samples_per_second < 100:
            score -= 0.1  # Poor performance
        
        # Chunk completion bonus
...
class ChunkedTrainer:
    ...
    def __init__(self,
                 neural_network: SLNetworkWrapper,
                 data_loader: FastDataLoader,
                 memory_threshold_gb: float = 32.0,
                 target_chunk_memory_gb: float = 16.0):
        """
    ...
    def train_chunked(self,
        ...
        # Initialize checkpoint manager with configurable per-chunk saving
        save_per_chunk = True  # Default value
        keep_n_checkpoints = 5  # Default value
        
        # Try to get configuration from preprocessed_training config if available
        # This would be passed from the main training script
        
        self.checkpoint_manager = ChunkedCheckpointManager(
            checkpoint_dir=checkpoint_dir,
            keep_n_checkpoints=keep_n_checkpoints,
            save_per_chunk=save_per_chunk
        )
        
        # Handle checkpoint resumption
        start_epoch = 0
        loss_history = []
        training_state = {}  # Initialize empty training state
        ...
        if auto_resume:
            ...
            if resume_checkpoint:
                try:
                    ...
                    # Check if this is a chunk checkpoint
                    if 'chunk_idx' in training_state:
                        chunk_idx = training_state['chunk_idx']
                        print(f"🔄 Resuming training from epoch {start_epoch}, chunk {chunk_idx}")
                        print(f"   Note: Will skip completed chunks in this epoch")
                        # Store chunk resume information for later use
                        training_state['resume_from_chunk'] = chunk_idx
                    else:
        ...
        try:
            ...
            for epoch in range(start_epoch, epochs):
                ...
                # Process each chunk (with resume support)
                resume_from_chunk = training_state.get('resume_from_chunk') if epoch == start_epoch else None
                for chunk in chunks:
                    # Skip chunks if resuming from a specific chunk
                    if resume_from_chunk is not None and chunk.chunk_id <= resume_from_chunk:
                        print(f"⏩ Skipping chunk {chunk.chunk_id} (already completed)")
                        continue
                    ...
                    # Save chunk checkpoint if enabled
                    if self.checkpoint_manager and self.checkpoint_manager.save_per_chunk:
                        try:
                            ...
                            chunk_stats = {
                                ...
                                'time': chunk_time,
                                'samples_per_second': chunk_samples / chunk_time if chunk_time > 0 else 0,
                                'completed': True,
                            ...
                            chunk_checkpoint_path = self.checkpoint_manager.save_chunk_checkpoint(
                                neural_network=self.neural_network,
                                epoch=epoch,
                                chunk_idx=chunk.chunk_id,
                                optimizer_state=optimizer.state_dict(),
                                loss_history=loss_history + [chunk_loss],  # Include current chunk loss
                            ...
                            if chunk_checkpoint_path:
                                logger.info(f"💾 Chunk checkpoint saved: {chunk_checkpoint_path}")
                        
                        except Exception as e:
                            logger.warning(f"⚠️ Failed to save chunk checkpoint: {e}")
                            print(f"   Chunk training continues...")
...
def main():
    ...
    neural_network = SLNetworkWrapper(model_args, device)
