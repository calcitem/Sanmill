...
class PreprocessedTrainingConfig:
    ...
    save_per_chunk: bool = True  # Save checkpoint after each chunk completion for better resume capability
    chunk_checkpoint_retention: int = 10  # Number of chunk checkpoints to keep
...
class SLConfig:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'SLConfig':
    ...
    @classmethod
    def load(cls, filepath: str) -> 'SLConfig':
...
def get_default_config() -> SLConfig:
...
def get_fast_training_config() -> SLConfig:
...
def get_production_config() -> SLConfig:
...
def get_cpu_optimized_config() -> SLConfig:
...
def get_gpu_optimized_config() -> SLConfig:
...
def get_preprocessed_training_config(mode: str = "default") -> SLConfig:
...
def validate_config(config: SLConfig) -> List[str]:
    """
...
def create_example_configs():
    ...
    for name, config in configs.items():
        # Save as YAML
        yaml_path = f"sl_config_{name}.yaml"
        config.save(yaml_path)
        print(f"Created {yaml_path}")

        # Save as JSON
        json_path = f"sl_config_{name}.json"
