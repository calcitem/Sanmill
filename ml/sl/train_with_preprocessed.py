...
try:
    ...
    from config import (get_default_config, get_fast_training_config,
                       get_preprocessed_training_config, SLConfig)
...
def load_config_from_file(config_path: str) -> SLConfig:
    """Load configuration from file."""
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"Configuration file not found: {config_path}")

    try:
        config = SLConfig.load(config_path)
...
def merge_config_with_args(config: SLConfig, args) -> SLConfig:
