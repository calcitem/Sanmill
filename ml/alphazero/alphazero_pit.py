#!/usr/bin/env python3
"""
AlphaZero Pitting Script for Sanmill - Human vs AlphaZero AI
Supports GUI interface for interactive games with AlphaZero model evaluation.

Usage:
  python alphazero_pit.py --config my_config.json --gui --first human
  python alphazero_pit.py --model model.tar --gui
  python alphazero_pit.py --model model.tar --games 5 --first ai
"""
...
# Import AlphaZero specific modules
try:
    from neural_network import AlphaZeroNet
    from mcts import MCTS
    from config import AlphaZeroConfig
except ImportError as e:
    print(f"Warning: AlphaZero modules not available: {e}")
    AlphaZeroNet = None
    MCTS = None
    AlphaZeroConfig = None
    ...
    def load_model(self):
        ...
        try:
            ...
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                ...
                if config_dict:
                    # Create model with saved configuration - pass as individual parameters
                    ...
                    # Use default configuration - pass as individual parameters
                    self.model = AlphaZeroNet(
                        input_channels=19,
                        num_filters=256,
...
class AlphaZeroGameGUI:
    ...
    def update_evaluation_display(self):
        ...
        if abs(self.current_evaluation) > 10:
            ...
            eval_text = f"AlphaZero Evaluation: {self.current_evaluation:+.1f} (Clear advantage)"
    ...
    def on_sims_changed(self, event=None):
        ...
        try:
            ...
            if 1 <= new_sims <= 10000:
                self.alphazero_player.set_mcts_sims(new_sims)
    ...
    def on_temp_changed(self, event=None):
        ...
        try:
            ...
            if 0.0 <= new_temp <= 2.0:
                self.alphazero_player.set_temperature(new_temp)
                logger.info(f"Temperature changed to: {new_temp}")
                self.update_status()
            else:
                # Reset to current value if invalid
                self.temp_var.set(f"{self.alphazero_player.temperature:.1f}")
        except ValueError:
            # Reset to current value if not a number
            self.temp_var.set(f"{self.alphazero_player.temperature:.1f}")
...
def main():
    parser = argparse.ArgumentParser(
        description='AlphaZero Pitting Script for Sanmill',
...
Examples:
  ...
  python alphazero_pit.py --create-config sample_config.json
    ...
    try:
        ...
        logger.info(f"Loading AlphaZero model from {model_path} on {device_str}")
        ...
        if use_gui:
            ...
            try:
                gui = AlphaZeroGameGUI(alphazero_player, human_first)
                gui.start_gui()
            except Exception as e:
            ...
            for game_num in range(args.games):
                ...
                while moves < 50 and game_state.get_valid_moves():
                    if game_state.side_to_move == (0 if human_first else 1):
                        ...
                        # AI move
                        move = alphazero_player.get_best_move(game_state)
                        if move:
