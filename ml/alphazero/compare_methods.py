...
Demonstrates why MCTS is beneficial for AlphaZero.
"""

import torch
import numpy as np
import time
from alphazero_pit import AlphaZeroGameAdapter, AlphaZeroPlayer, AlphaZeroModelLoader
...
def mcts_evaluation_method(alphazero_player, game_state):
    """MCTS-guided search - AlphaZero approach"""
    print("\n🌳 MCTS Search Method:")
    ...
    # Reset statistics
    alphazero_player.model_evaluations = 0
    alphazero_player.mcts_calls = 0

    # Get best move using MCTS (or fallback)
    best_move = alphazero_player.get_best_move(game_state)

    elapsed = time.time() - start_time
    evaluations = alphazero_player.model_evaluations

    print(f"- MCTS simulations: {alphazero_player.mcts_sims}")
    print(f"- Model evaluations: {evaluations}")
    print(f"- Time: {elapsed:.3f}s")
    print(f"- Selected move: {best_move}")

    # Get position evaluation for comparison
    value, _ = alphazero_player.evaluate_position(game_state)
...
def main():
    print("🎯 AlphaZero: Direct NN vs MCTS Comparison")
    print("=" * 50)
    ...
    try:
        ...
        model_loader = AlphaZeroModelLoader(model_path, 'cpu')  # Use CPU for fair comparison
        model = model_loader.load_model()

        # Create AlphaZero player with reduced MCTS sims for demonstration
        alphazero_player = AlphaZeroPlayer(model_loader, mcts_sims=100, temperature=0.0)

        # Create test positions
        test_positions = []

        # Position 1: Opening position
        game_state1 = AlphaZeroGameAdapter()
        test_positions.append(("Opening Position", game_state1))

        # Position 2: Mid-game position (simulate some moves)
        game_state2 = AlphaZeroGameAdapter()
        ...
        for position_name, game_state in test_positions:
            ...
            try:
                mcts_result = mcts_evaluation_method(alphazero_player, game_state)
