...
class Game {
  ...
  bool doMove(ExtMove extMove) {
    logger.i('[game] doMove: $extMove');

    // Add validation for place moves
    if (extMove.type == MoveType.place) {
      final int targetSquare = extMove.to;
      if (targetSquare >= 0 && targetSquare < sqNumber) {
        // Check if the target square is occupied
        final PieceColor pieceAtSquare =
            GameController().position.pieceOnGrid(squareToIndex[targetSquare]!);
        if (pieceAtSquare != PieceColor.none) {
          logger.e(
              '[game] Cannot place at ${extMove.move}: square already occupied by $pieceAtSquare');
          logger.e(
              '[game] Current board state: ${GameController().position.fen}');
    ...
    try {
      ...
      bool success = GameController().position.doMove(extMove.move);

      if (!success) {
        logger.e('[game] doMove failed for move: $extMove');
        logger.e(
            '[game] Move type: ${extMove.type}, Move string: ${extMove.move}');
        logger
            .e('[game] Current position FEN: ${GameController().position.fen}');
        throw EngineNoBestMove();
      }
      ...
      logger.e('[game] Error during doMove: $e');
      throw e;
    }
  }
