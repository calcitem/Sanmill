...
class _PuzzleCreationPageState extends State<PuzzleCreationPage>
  ...
  Future<void> _openGameBoard(GameMode mode) async {
    ...
    if (_snapshottedPosition != null) {
      ...
      if (!_isRecordingSolution || mode == GameMode.setupPosition) {
        controller.activeNativeMillSession?.loadFen(_snapshottedPosition!);
      }
  ...
  void _checkAndSnapshotPosition() {
    final GameController controller = GameController();
    final String fen = controller.activeBoardView.fen ?? '';
    ...
    // Validate FEN
    if (!PuzzleRuleEngine.isValidFen(fen)) {
      return;
    }
  ...
  void _stopRecordingSolution() {
    ...
    final String? finalFen = controller.activeBoardView.fen;
  ...
  String? _validatePuzzle() {
    ...
    if (!PuzzleRuleEngine.isValidFen(_snapshottedPosition!)) {
