...
class PuzzleValidator {
  PuzzleValidator({required this.puzzle}) {
    ...
    final MillBoardView initial = engine?.view ?? MillBoardView.empty();
  ...
  bool _checkObjective(MillBoardView position) {
  ...
  bool _checkMillFormed(MillBoardView position) {
    return _millFormedDuringAttempt || position.action == Act.remove;
  }

  /// Check if required pieces were captured
  bool _checkPiecesCaptured(MillBoardView position) {
  ...
  static int _countPieces(MillBoardView position, PieceColor side) {
