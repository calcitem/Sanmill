...
void main() {
  ...
  group('Custodian and Intervention Validation', () {
    ...
    test('FEN validation rejects invalid custodian targets (FR-035)', () {
      ...
      expect(fenSet, isFalse, reason: 'Invalid custodian FEN should be rejected per FR-035');
    });
    ...
    test('FEN validation rejects invalid intervention targets (FR-035)', () {
      ...
      expect(fenSet, isFalse, reason: 'Invalid intervention FEN should be rejected per FR-035');
    });

    test('Valid FEN without capture markers imports successfully', () {
      // Test basic FEN import/export functionality
      const String validFen = '********/********/******** w p p 0 12 0 12 0 0 0 0 0 0 0 0 1';
      ...
      expect(exportedFen, isNot(contains('i:')), reason: 'No intervention marker');
    });

    test('mayRemoveMultiple setting is correctly configured', () {
      // Verify that the rule settings are properly configured
      expect(DB().ruleSettings.enableCustodianCapture, isTrue, 
             reason: 'Custodian capture should be enabled');
      expect(DB().ruleSettings.enableInterventionCapture, isTrue,
      ...
      expect(DB().ruleSettings.hasDiagonalLines, isTrue,
    ...
    test('Position state management with capture counts', () {
      ...
      expect(position.pieceToRemoveCount[PieceColor.white], equals(0),
             reason: 'Initial white remove count should be 0');
      expect(position.pieceToRemoveCount[PieceColor.black], equals(0),
    ...
    test('FEN export includes capture markers when active', () {
      ...
      const String baseFen = '*@******/********/******** w p r 1 11 1 11 0 1 0 0 0 0 0 0 1';
