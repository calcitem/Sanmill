...
void main() {
  ...
  /// Helper: build a read-only [MillBoardView] from a FEN through the native
  /// Rust kernel (replaces the deleted legacy `Position` parser).
  MillBoardView viewOf(String fen) {
    final PuzzleRuleEngine engine = PuzzleRuleEngine.tryLoad(fen)!;
    final MillBoardView view = engine.view;
    engine.dispose();
    return view;
  }
  ...
  group('PuzzleValidator mill formation tracking', () {
    test('validator starts with mill flag unset', () {
      ...
      final MillBoardView pos = viewOf(puzzle.initialPosition);
