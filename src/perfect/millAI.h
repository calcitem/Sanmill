...
public:
    ...
    unsigned int
        posTo[MAX_NUM_POS_MOVES]; // target board position of a possible move
    unsigned int
        posFrom[MAX_NUM_POS_MOVES]; // source board position of a possible move
...
public:
    ...
    static const unsigned int noWarning
        = 0; // so the bitwise or-operation can be applied, without interacting
             // with playerOne & Two
    ...
    unsigned int
        warnings[size]; // array containing the warnings for each board position
...
private:
    ...
    void setNeighbour(unsigned int index, unsigned int firstNeighbour0,
        unsigned int secondNeighbour0, unsigned int firstNeighbour1,
