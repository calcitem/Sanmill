    ...
    int blackStonesToPlace, int playerToMove, bool onlyStoneTaking)
{
    try {
        ...
        gameState.phase = ((whiteStonesToPlace == 0 &&
        ...
        std::string evalStr =
            MalomSolutionAccess::perfectPlayer->evaluate(gameState).to_string();
        ...
        // Parse evaluation string format: "W, (228, 75)" where 75 is the step
        // count
        if (!evalStr.empty()) {
            ...
            if (lastParen != std::string::npos) {
                ...
                if (commaPos != std::string::npos &&
                    ...
                    std::string stepSub = evalStr.substr(
                        commaPos + 1, closePos - commaPos - 1);
                    // Trim whitespace
                    ...
                    if (std::regex_search(stepSub, m, std::regex("-?\\d+"))) {
                        try {
                            ...
                            debugPrintf("Parsed step count: %d from string: "
                                        "'%s'\n",
...
PerfectEvaluation getDetailedEvaluation(const Position &position)
{
    try {
        ...
        PerfectEvaluation result = MalomSolutionAccess::get_detailed_evaluation(
            whiteBitboard, blackBitboard, whiteStonesToPlace,
            blackStonesToPlace, playerToMove, onlyStoneTaking);
