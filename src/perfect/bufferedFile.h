...
private:
    ...
    unsigned char*
        readBuffer; // Array of size [numThreads*blockSize] containing the data
                    // of the block, where reading is taking place
    unsigned char* writeBuffer; // '' - access by [threadNo*bufferSize+position]
    int64_t* curReadingPointer; // array of size [numThreads] with pointers to
