...
bool MiniMax::initAlphaBeta(AlphaBetaGlobalVars& alphaBetaVars)
{
    ...
    invalidArray = new BufferedFile(threadManager.getNumThreads(),
        FILE_BUFFER_SIZE, ssInvArrayFilePath.str().c_str());

    if (invalidArray->getFileSize()
        == (LONGLONG)layerStats[alphaBetaVars.layerNumber].knotsInLayer) {
        PRINT(2, this,
            "  Loading invalid states from file: " << ssInvArrayFilePath.str());
    ...
    ThreadManager::ThreadVarsArray<InitAlphaBetaVars> tva(
        ...
        (InitAlphaBetaVars&)InitAlphaBetaVars(this, &alphaBetaVars,
            alphaBetaVars.layerNumber, invalidArray, initAlreadyDone));

    // process each state in the current layer
    switch (threadManager.executeParallelLoop(initAlphaBetaThreadProc,
        tva.getPointerToArray(), tva.getSizeOfArray(), TM_SCHEDULE_STATIC, 0,
...
DWORD MiniMax::initAlphaBetaThreadProc(void* pParameter, unsigned int index)
{
    ...
    if (iabVars->initAlreadyDone) {
        ...
        if (!m->setSituation(iabVars->curThreadNo, curState.layerNumber,
            ...
            m->getValueOfSituation(
                iabVars->curThreadNo, floatValue, curStateValue);
        }
    }
    ...
    m->savePlyInfoInDatabase(
        curState.layerNumber, curState.stateNumber, plyInfo);
...
DWORD MiniMax::runAlphaBetaThreadProc(void* pParameter, unsigned int index)
{
    ...
    // set current selected situation
    if (m->setSituation(
            rabVars->curThreadNo, curState.layerNumber, curState.stateNumber)) {
        // calc value of situation
        m->letTheTreeGrow(&root, rabVars, m->depthOfFullTree,
            SKV_VALUE_GAME_LOST, SKV_VALUE_GAME_WON);
    } else {
        // should not occur, because already tested by plyInfo ==
        // PLYINFO_VALUE_UNCALCULATED
        MessageBoxW(nullptr,
            L"This event should never occur. if (!m->setSituation())", L"ERROR",
            MB_OK);
    }
    return TM_RETURN_VALUE_OK;
}
    ...
    unsigned int tilLevel, float alpha, float beta)
{
    ...
    // standard values
    knot->branches
        = &rabVars->branchArray[(depthOfFullTree - tilLevel) * maxNumBranches];
    ...
    if (tilLevel == 0) {
        if (calcDatabase) {
            ...
            getValueOfSituation(
                rabVars->curThreadNo, knot->floatValue, knot->shortValue);
        }
        ...
        if (alphaBetaTryDataBase(
                knot, rabVars, tilLevel, layerNumber, stateNumber))
            return;
        ...
        if (knot->numPossibilities == 0) {
            ...
            alphaBetaTryPossibilities(knot, rabVars, tilLevel, idPossibility,
                pPossibilities, maxWonfreqValuesSubMoves, alpha, beta);
        ...
        if (calcDatabase && hFileShortKnotValues && hFilePlyInfo)
            alphaBetaSaveInDatabase(rabVars->curThreadNo, layerNumber,
                stateNumber, knot->shortValue, knot->plyInfo,
...
bool MiniMax::alphaBetaTryDataBase(Knot* knot, RunAlphaBetaVars* rabVars,
    unsigned int tilLevel, unsigned int& layerNumber, unsigned int& stateNumber)
{
    // locals
    ...
    if (hFilePlyInfo != nullptr && hFileShortKnotValues != nullptr
        ...
        readKnotValueFromDatabase(rabVars->curThreadNo, layerNumber,
            stateNumber, shortKnotValue, invalidLayerOrStateNumber,
        ...
        if ((tilLevel < depthOfFullTree && invalidLayerOrStateNumber)
            ...
            || (tilLevel < depthOfFullTree
                && shortKnotValue == SKV_VALUE_INVALID
                && plyInfo
                    != PLYINFO_VALUE_UNCALCULATED)) { // version 22: replaced:
                                                      // curCalculatedLayer ==
                                                      // layerNumber &&
                                                      // knot->plyInfo !=
                                                      // PLYINFO_VALUE_UNCALCULATED))
        ...
        // value
        if (shortKnotValue != SKV_VALUE_INVALID && tilLevel == depthOfFullTree
            && !calcDatabase && subLayerInDatabaseAndCompleted) {
            PRINT(2, this,
                "This state is marked as " << ((shortKnotValue
                                                   == SKV_VALUE_GAME_WON)
                        ? "WON"
        ...
        if ((calcDatabase && tilLevel < depthOfFullTree
                && shortKnotValue != SKV_VALUE_INVALID
                && plyInfo != PLYINFO_VALUE_UNCALCULATED)
            || (!calcDatabase && tilLevel < depthOfFullTree - 1
                && shortKnotValue != SKV_VALUE_INVALID)) {
            // switch if is not opponent level
            if (knot->isOpponentLevel)
                knot->shortValue = skvPerspectiveMatrix[shortKnotValue]
...
void MiniMax::alphaBetaTryPossibilities(Knot* knot, RunAlphaBetaVars* rabVars,
    unsigned int tilLevel, unsigned int* idPossibility, void* pPossibilities,
    unsigned int& maxWonfreqValuesSubMoves, float& alpha, float& beta)
{
    // locals
    void* pBackup;
    ...
    for (curPoss = 0; curPoss < knot->numPossibilities; curPoss++) {
        ...
        if (tilLevel == depthOfFullTree && !calcDatabase) {
            printMoveInformation(
                rabVars->curThreadNo, idPossibility[curPoss], pPossibilities);
        ...
        // recursive call
        letTheTreeGrow(
            &knot->branches[curPoss], rabVars, tilLevel - 1, alpha, beta);
        ...
        if (tilLevel == depthOfFullTree && !calcDatabase) {
            ...
            if (hFileShortKnotValues != nullptr && layerInDatabase) {
                storeValueOfMove(rabVars->curThreadNo, idPossibility[curPoss],
                    pPossibilities, knot->branches[curPoss].shortValue,
...
void MiniMax::alphaBetaCalcPlyInfo(Knot* knot)
{
    ...
    if (knot->shortValue == SKV_VALUE_GAME_DRAWN) {
        ...
        shortKnotValue = (knot->isOpponentLevel)
            ? skvPerspectiveMatrix[knot->shortValue][PL_TO_MOVE_UNCHANGED]
            : knot->shortValue;
        maxPlyInfo
            = (shortKnotValue == SKV_VALUE_GAME_WON) ? PLYINFO_VALUE_DRAWN : 0;
        maxBranch = 0;
        ...
        if (shortKnotValue == SKV_VALUE_GAME_WON) {
            for (i = 0; i < knot->numPossibilities; i++) {
                ...
                shortKnotValue = (knot->branches[i].isOpponentLevel)
