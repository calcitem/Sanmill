...
struct RetroAnalysisQueueState {
    StateNumberVarType
        stateNumber; // state stored in the retro analysis queue. the queue is a
                     // buffer containing states to be passed to
...
{
    vector<CyclicArray*>
        statesToProcess; // vector-queue containing the states, whose short knot
                         // value are known for sure. they have to be processed.
                         // if processed the state will be removed from list.
    ...
    vector<vector<RetroAnalysisQueueState>>
        stateQueue; // Queue containing states, whose 'count value' shall be
                    // increased by one. Before writing 'count value' to 'count
                    // array' the writing positions are sorted for faster
                    // processing.
...
struct RetroAnalysisVars // constant during calculation
{
    vector<CountArrayVarType*>
        countArrays; // One count array for each layer in 'layersToCalculate'.
                     // (For the nine men's morris game two layers have to
                     // considered at once.)
