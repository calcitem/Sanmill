...
bool checkNNUEFileHeader(const std::string &filePath)
{
    ...
    if (!stream.is_open()) {
        sync_cout << "info string ERROR: Cannot open file for header check: "
                  << filePath << sync_endl;
        return false;
    }
    ...
    sync_cout << "info string NNUE file version: 0x" << std::hex << fileVersion
              << std::dec << sync_endl;

    // Expected version from nnue_common.h
    const std::uint32_t expectedVersion = 0x7AF32F20u;
    sync_cout << "info string Expected version: 0x" << std::hex
              << expectedVersion << std::dec << sync_endl;

    if (fileVersion != expectedVersion) {
        sync_cout << "info string WARNING: Version mismatch! File version: 0x"
                  << std::hex << fileVersion << ", Expected: 0x"
                  << expectedVersion << std::dec << sync_endl;
    }

    // Read hash value
    std::uint32_t hashValue = readLittleEndianUint32(stream);
    sync_cout << "info string NNUE file hash value: 0x" << std::hex << hashValue
              << std::dec << sync_endl;

    // Read description size
    std::uint32_t descSize = readLittleEndianUint32(stream);
    sync_cout << "info string NNUE description size: " << descSize << " bytes"
              << sync_endl;
    ...
    if (descSize > 0 && descSize < 1000) { // Reasonable size check
        ...
        if (stream.gcount() == static_cast<std::streamsize>(descSize)) {
            sync_cout << "info string NNUE description: " << description
                      << sync_endl;
        }
    }
...
void init_nnue()
{
    if (!evalFile.empty()) {
        ...
        // Normalize the file path to use correct separators for the current
        // platform
        ...
        if (nnueInitialized && normalizedPath == loadedNnuePathNormalized) {
            sync_cout << "info string NNUE already initialized with the same "
                         "model; skipping re-init"
                      << sync_endl;
            return;
        }
        ...
        sync_cout << "info string Original NNUE model path: " << evalFile
                  << sync_endl;
        sync_cout << "info string Normalized NNUE model path: "
                  << normalizedPath << sync_endl;
        sync_cout << "info string Attempting to load NNUE model from: "
                  << normalizedPath << sync_endl;

        // Check if file can be opened
        if (!stream.is_open()) {
            sync_cout << "info string ERROR: Failed to open NNUE model file: "
                      << normalizedPath << sync_endl;
            sync_cout << "info string NNUE initialization failed - model file "
                         "not accessible"
                      << sync_endl;
        ...
        sync_cout << "info string NNUE model file size: " << fileSize
                  << " bytes" << sync_endl;

        if (fileSize == 0) {
            sync_cout << "info string ERROR: NNUE model file is empty"
                      << sync_endl;
            sync_cout << "info string NNUE initialization failed - empty model "
                         "file"
                      << sync_endl;
            nnueInitialized = false;
            return;
        ...
        bool loadResult = Stockfish::Eval::NNUE::load_eval(normalizedPath,
                                                           stream);
        if (!loadResult) {
            sync_cout << "info string ERROR: Failed to load NNUE model - "
                         "checking specific failure point..."
                      << sync_endl;
            ...
            sync_cout << "info string Diagnostic - Version: 0x" << std::hex
                      << testVersion << ", Hash: 0x" << testHashValue
                      << ", DescSize: " << std::dec << testDescSize
                      << sync_endl;
            ...
            sync_cout << "info string Current stream position after header: "
                      << currentPos << sync_endl;

            // Check remaining bytes
            stream.seekg(0, std::ios::end);
            std::streampos endPos = stream.tellg();
            sync_cout << "info string Remaining bytes for parameters: "
                      << (endPos - currentPos) << sync_endl;

            // Check if the issue is with FeatureTransformer dimensions
            sync_cout << "info string Expected FeatureTransformer input "
                         "dimensions: "
                      << Stockfish::Eval::NNUE::FeatureSet::Dimensions
                      << sync_endl;
            sync_cout << "info string Expected TransformedFeatureDimensions: "
                      << Stockfish::Eval::NNUE::TransformedFeatureDimensions
                      << sync_endl;

            // Calculate expected parameter sizes
            const auto featureDims =
                Stockfish::Eval::NNUE::FeatureSet::Dimensions;
            const auto transformedDims =
                Stockfish::Eval::NNUE::TransformedFeatureDimensions;
            const auto psqtBuckets = Stockfish::Eval::NNUE::PSQTBuckets;

            sync_cout << "info string Expected parameter sizes:" << sync_endl;
            sync_cout << "info string - Biases: " << transformedDims
                      << " elements" << sync_endl;
            sync_cout << "info string - Weights: "
                      << (transformedDims * featureDims) << " elements"
                      << sync_endl;
            sync_cout << "info string - PSQT Weights: "
                      << (psqtBuckets * featureDims) << " elements"
                      << sync_endl;

            // Calculate expected bytes (actual serializer writes int16 bias and
            // int16 FT weights, int32 PSQT)
            const size_t biasBytes = transformedDims * 2; // int16_t
            const size_t weightBytes = transformedDims * featureDims *
                                       2;                           // int16_t
            const size_t psqtBytes = psqtBuckets * featureDims * 4; // int32_t
            const size_t totalFeatureTransformerBytes = biasBytes +
                                                        weightBytes + psqtBytes;

            sync_cout << "info string Expected FeatureTransformer bytes:"
                      << sync_endl;
            sync_cout << "info string - Biases: " << biasBytes << " bytes"
                      << sync_endl;
            sync_cout << "info string - Weights: " << weightBytes << " bytes"
                      << sync_endl;
            sync_cout << "info string - PSQT Weights: " << psqtBytes << " bytes"
                      << sync_endl;
            sync_cout << "info string - Total FeatureTransformer: "
                      << totalFeatureTransformerBytes << " bytes" << sync_endl;

            // Calculate network layer bytes (InputLayer: 1536 -> HiddenLayer1:
            // 15 -> HiddenLayer2: 32 -> OutputLayer: 1) Each layer has biases
            // (int32_t, 4 bytes) and weights (int8_t, 1 byte)
            const size_t layer1BiasBytes = 15 * 4; // 15 biases, int32_t
            // nnue-pytorch writes an extra (L2+1) row in L1; account for it in
            // expected size
            const size_t layer1WeightBytes = (15 + 1) * transformedDims *
                                             1;           // 16 * 1536
            const size_t layer2BiasBytes = 32 * 4;        // 32 biases, int32_t
            const size_t layer2WeightBytes = 32 * 15 * 1; // 32 * 15 weights,
                                                          // int8_t
            const size_t layer3BiasBytes = 1 * 4;         // 1 bias, int32_t
            const size_t layer3WeightBytes = 1 * 32 * 1;  // 1 * 32 weights,
                                                          // int8_t

            const size_t totalNetworkBytes = layer1BiasBytes +
                                             layer1WeightBytes +
                                             layer2BiasBytes +
                                             layer2WeightBytes +
                                             layer3BiasBytes +
            ...
            sync_cout << "info string Expected Network layer bytes:"
                      << sync_endl;
            sync_cout << "info string - Layer1 (1536->15): "
                      << (layer1BiasBytes + layer1WeightBytes) << " bytes"
                      << sync_endl;
            sync_cout << "info string - Layer2 (15->32): "
                      << (layer2BiasBytes + layer2WeightBytes) << " bytes"
                      << sync_endl;
            sync_cout << "info string - Layer3 (32->1): "
                      << (layer3BiasBytes + layer3WeightBytes) << " bytes"
                      << sync_endl;
            sync_cout << "info string - Total Network: " << totalNetworkBytes
                      << " bytes" << sync_endl;

            const size_t totalExpectedBytes =
                totalFeatureTransformerBytes +
                totalNetworkBytes * Stockfish::Eval::NNUE::LayerStacks;
            sync_cout << "info string Total expected bytes (with "
                      << Stockfish::Eval::NNUE::LayerStacks
                      << " layer stacks): " << totalExpectedBytes << " bytes"
                      << sync_endl;
            sync_cout << "info string Available bytes: "
                      << (endPos - currentPos) << " bytes" << sync_endl;

            if (featureDims != 1152) {
                sync_cout << "info string WARNING: FeatureSet dimensions ("
                          << featureDims
                          << ") do not match expected Nine Men's Morris "
                             "dimensions (1152)"
                          << sync_endl;
            }

            if (transformedDims != featureDims) {
                sync_cout << "info string ERROR: TransformedFeatureDimensions ("
                          << transformedDims
                          << ") does not match FeatureSet dimensions ("
                          << featureDims << ")" << sync_endl;
            } else {
                sync_cout << "info string OK: Dimensions are consistent ("
                          << transformedDims << ")" << sync_endl;
            }

            sync_cout << "info string NNUE model loading failed - parameters "
                         "could not be loaded"
                      << sync_endl;
            nnueInitialized = false;
            return;
        ...
        sync_cout << "info string NNUE model successfully loaded from "
                  << normalizedPath << sync_endl;
...
Value Eval::evaluate(Position &pos)
{
    ...
    if (useNNUE) {
        if (nnueInitialized) {
            ...
            sync_cout << "info string ERROR: NNUE is enabled but not "
